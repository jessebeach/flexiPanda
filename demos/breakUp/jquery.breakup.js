      ...
      for (br in options) {
        if (options.hasOwnProperty(br)) {
          ...
          if (typeof options[br] === 'function') {
            ...
            breakPoints[index] = buildProxy(options[br], this);
          }
          else {
	...
	var methods = {
		init : function (opts) {
			...
			return this.each(function () {
				var $this = $(this);
        setBreakPoints.call($this, options);
        // Register a custom 'breakChanged' event on the document.
        $this.bind('breakChanged' + '.' + plugin, {options: options}, breakChangeHandler);
        // Register a handler on the window resize event.
        var f = buildProxy(breakCheck, $this);
        $(window).bind('resize' + '.' + plugin, {options: options}, f);
        $(window).bind('load' + '.' + plugin, {options: options}, f);
			});
		},
		destroy : function () {},
		unset: function () {}
